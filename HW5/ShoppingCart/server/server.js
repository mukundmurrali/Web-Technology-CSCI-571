...
app.get('/searchResults', function (req, res) {
    try {
        ...
        if (req.query.enablenearbysearch) {
            var miles = 10;
            if (req.query.miles) {
        ...
        //console.log("SEARCH APICALL" + apicall);
                    ...
                    if (body.findItemsAdvancedResponse[0].searchResult[0].item) {
                        ...
                        items.forEach(function(item) {
                            ...
                            if (item.condition && item.condition.length > 0) {
                                if (item.condition[0].conditionId && item.condition[0].conditionId.length > 0) {
                                    conditionId = parseInt(item.condition[0].conditionId[0], 10);
                                }
                                if (conditionId == 1000) {
                                    condition = "NEW";
                            ...
                            itemsArr.push({"index" : index, "itemId" : itemId, "image" : photo, "title" : name, "subtitle" : subtitle, "price" : price, "shipping_cost" : ship_option, "zipcode" : zipcode, "sellerName" : sellerName, "condition" : condition});
                        });
                    }
                }
            }
...
app.get('/getDetails', function (req, res) {
    try {
        ...
        //console.log("PRODUCT DETAILS APICALL" + apicall);
        request(apicall, { json: true }, (err, resp, body) => {
            if (err) { return console.log(err); }
            if (body.Ack == "Success") {
                if (body.Item) {
                    ...
                    if (itemDetails.ReturnPolicy) {
                        ...
                        if (itemDetails.ReturnPolicy.ReturnsWithin) {
                            returnswithin = itemDetails.ReturnPolicy.ReturnsWithin;
                        }
                        if (itemDetails.ReturnPolicy.Refund) {
                            refundmode = itemDetails.ReturnPolicy.Refund;
                        }
                        if (itemDetails.ReturnPolicy.ShippingCostPaidBy) {
                            shippedBy = itemDetails.ReturnPolicy.ShippingCostPaidBy;
                        }
                        returnpolicyObj["policy"] = returnpolicy;
                        returnpolicyObj["returns_within"] = returnswithin;
                        returnpolicyObj["refund_mode"] = refundmode;
                        returnpolicyObj["shipped_by"] = shippedBy;
                    }
                    ...
                    var globalShipping = "N/A";
                    if (itemDetails.GlobalShipping) {
                        globalShipping = itemDetails.GlobalShipping;
                    }
                    
                    var handlingTime = "N/A";
                    if (itemDetails.HandlingTime) {
                        handlingTime = itemDetails.HandlingTime;
                    ...
                    var condition = "N/A";
                    if (itemDetails.ConditionDisplayName) {
                        condition = itemDetails.ConditionDisplayName;
                    }
                    var shippingInfo = {"global_shipping" : globalShipping, "handling_time" : handlingTime, "condition" : condition};

                    var productDetails = {"Photo" : pictureURL, "Title" : title, "Price" : price, "Location" : location, "Return_Policy" : returnpolicyObj, "ItemSpecifics" : itemspecifics, "seller_tab" : sellerTab, "facebook_link": ViewItemURLForNaturalSearch, "shipping_info" : shippingInfo};

                    var googleapicall = "https://www.googleapis.com/customsearch/v1?";
                    googleapicall += "q=" + title + "&cx=016586471237920341027:ato2kofot4q&imgSize=huge&imgType=news&num=8&searchType=image&key=AIzaSyBjM4qQGzfirozmWJyux-s6ZEuhAqtdfkU";
                    ...
                    //console.log("SIMILAR ITEMS CALL" + similarItemsCall);
